...
<img src="{{ '../assets/images/FTA/FTA-quartic-real.png' | relative_url }}" width="500" height="156" />
...
Now, having gotten complex numbers into our head, how can we look for a complex root of \\(q\\)? 

Just as we tried taking a far-away positive point and a far-away negative point to surround \\(0\\) in the real numbers, let's try taking a far-away point in *every* direction to surround \\(0\\) in the complex numbers. That is, for each \\(0 \le \theta \le 2\pi\\), let's take \\(z = 100 e^{i \theta}\\). The corresponding value of \\(q(z)\\) is

$$ q(100 e^{i \theta}) = 100000000 e^{4i \theta} + 1000000 e^{3i\theta} + 100e^{i\theta} + 2.$$

Notice how the first term is *much* bigger than all the rest of the terms. That is, for \\(z\\) far-away, \\(q(z)\\) looks roughly like \\(z^4\\).
...
In fact, this method works to show that *any* polynomial \\(q(z)\\) has a root in the complex numbers.